...
namespace WindowsFormsApplication2.Storage
{
    ...
    public class SentData : Database
    {
        ...
        public override void Init()
        {
            ...
            //? add "condition_val" column to old table
            this.cmd.CommandText = "SELECT * FROM sqlite_master WHERE name='sent' and sql like '%condition_val%'";
            object readState3 = this.cmd.ExecuteScalar();
            if (readState3 == null)
            {
                this.cmd.CommandText = "ALTER TABLE sent ADD COLUMN condition_val TEXT DEFAULT '0,0,0'";
                this.cmd.ExecuteNonQuery();
            }

            //? add "phrases_now" colulmn to old table
            this.cmd.CommandText = "SELECT * FROM sqlite_master WHERE name='sent' and sql like '%phrases_now%'";
            object readState4 = this.cmd.ExecuteScalar();
            if (readState4 == null)
            {
                this.cmd.CommandText = "ALTER TABLE sent ADD COLUMN phrases_now TEXT DEFAULT '[]'";
                this.cmd.ExecuteNonQuery();
            }
        }

        public long InsertSent(string create_time, string article, string full_text, string title, string phrases, string separator, int type, int disorder, int no_repeat, int count, int mark, string segment_record, int segment_cursor, int cur_segment_num, int sent_num, int sent_count, int cycle, int cycle_value, int auto, int auto_condition, int auto_no, int trim, int ph_order, string condition_val, string phrases_now)
        {
            this.cmd.CommandText = $"INSERT INTO sent VALUES(NULL,'{create_time}','{ConvertText(article)}','{ConvertText(full_text)}','{ConvertText(title)}','{ConvertText(phrases)}','{ConvertText(separator)}',{type},{disorder},{no_repeat},{count},{mark},'{ConvertText(segment_record)}',{segment_cursor},{cur_segment_num},{sent_num},{sent_count},{cycle},{cycle_value},{auto},{auto_condition},0,0,0,{auto_no},{trim},{ph_order},'{ConvertText(condition_val)}','{ConvertText(phrases_now)}'); SELECT last_insert_rowid();";
            object readId = this.cmd.ExecuteScalar();
            return Convert.ToInt64(readId);
        }

        public void UpdateSent(long id, string full_text, string title, int mark, string segment_record, int segment_cursor, int cur_segment_num, int sent_num, int sent_count, int cycle, int cycle_value, int auto, int auto_condition, int auto_no, string condition_val, string phrases_now)
        {
            this.cmd.CommandText = $"UPDATE sent SET full_text='{ConvertText(full_text)}', title='{ConvertText(title)}', mark={mark}, segment_record='{ConvertText(segment_record)}', segment_cursor={segment_cursor}, cur_segment_num={cur_segment_num}, sent_num={sent_num}, sent_count={sent_count}, cycle={cycle}, cycle_value={cycle_value}, auto={auto}, auto_condition={auto_condition}, auto_no={auto_no}, condition_val='{ConvertText(condition_val)}', phrases_now='{ConvertText(phrases_now)}' WHERE id={id};";
            this.cmd.ExecuteNonQuery();
        }
